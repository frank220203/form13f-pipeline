...
@pytest.mark.anyio
async def test_load_tickers(mock_ticker_repository: MagicMock, mock_submission_repository: MagicMock) -> None:
    # Given & Mock
    mock_ticker_repository.add_data = AsyncMock()
    mock_ticker_repository.add_data.return_value = 1

    # When & Mocking
    pipeline_usecase = PipelineUsecase(mock_ticker_repository, mock_submission_repository)
...
@pytest.mark.anyio
async def test_load_submissions(mock_ticker_repository: MagicMock, mock_submission_repository: MagicMock) -> None:
    # Given & Mock
